-- Opfølgning på kodegennemgang af e-conomic-arbejdet (2026-09-14).
--
-- 1) Det planlagte fakturanummer-opslag (cron 'operia-economic-sync') havde
--    intet sted at sige, at det fejlede: svaret gik til net.http_post, som
--    smider det væk. Nu skriver kørslen status på integrationens række, som
--    de andre planlagte kørsler (booking-eksport, Dalux) gør på deres.
--
-- 2) transfer_invoice_draft får p_detail: overførslen kan lægge sit eget i
--    revisionsrækken. Bruges til momstilsidesættelsen ("Overfør alligevel"):
--    hvilke linjer, produkter og koder bogholderen valgte at sende trods
--    afvigelsen — det stod kun i browserens hukommelse. Den gamle signatur
--    droppes eksplicit (PGRST203 ved to overloads).

alter table public.company_accounting_config
  add column if not exists sync_last_run_at timestamptz,
  add column if not exists sync_last_run_status text
    check (sync_last_run_status is null or sync_last_run_status in ('ok', 'failed')),
  add column if not exists sync_last_run_error text;

drop function if exists public.transfer_invoice_draft(uuid, text, text, text);

CREATE OR REPLACE FUNCTION public.transfer_invoice_draft(p_draft_id uuid, p_invoice_no text, p_system text DEFAULT 'manual'::text, p_external_id text DEFAULT NULL::text, p_detail jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  perform public.record_audit(d.company_id, 'invoice_draft.transferred', 'invoice_draft',
    ...
    jsonb_build_object('system', coalesce(v_sys, 'manual')) || coalesce(p_detail, '{}'::jsonb));
end;
$function$;

revoke all on function public.transfer_invoice_draft(uuid, text, text, text, jsonb) from public;
grant execute on function public.transfer_invoice_draft(uuid, text, text, text, jsonb) to authenticated;
