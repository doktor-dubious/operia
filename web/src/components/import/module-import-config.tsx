...
// Aktiv-/lagerimportens konfigurationsside — samme UI som medarbejder-
// importens (Import → Konfiguration), blot med modulets egne felter.
// Nøglefeltet (aktiv-nr./varenr.) og navn er obligatoriske og kan ikke
// fjernes, men gerne flyttes.
...
export function ModuleImportConfig({ spec }: { spec: ModuleSpec }) {
  const { t } = useTranslation()
  const { companyId } = useCompanyContext()
  const { data, isPending } = useImportConfig(companyId, spec.importType)
  const queryClient = useQueryClient()
  const [values, setValues] = useState<Values | null>(null)
  const [saving, setSaving] = useState(false)

  const allFields = spec.fields.map((f) => f.key)
  ...
  const save = async () => {
    ...
    queryClient.invalidateQueries({ queryKey: ['import-config', companyId, spec.importType] })
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid max-w-2xl gap-6 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">{t('importConfig.potentialFieldsHint')}</p>
            <div className="flex flex-col gap-1.5">
              {inactive.length === 0 ? (
